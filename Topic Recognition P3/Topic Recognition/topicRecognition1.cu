...
std::map<char, thrust::device_vector<int>> createItemMemory(std::map<char, thrust::device_vector<int>> iM)
{
...
thrust::device_vector<int> lookUpitemMemory(std::map<char, thrust::device_vector<int>> iM, char key)
{
	thrust::device_vector<int> randomHV(D);
	randomHV = iM[key];
	return randomHV;
}
...
std::vector<int> computeSumHV(std::map<char, thrust::device_vector<int>> iM, size_t bufferSize, std::string  buffer)
{
	//std::vector<int> h_block0(D, 1);
	thrust::device_vector<int> st_block2(D, 1);
	thrust::host_vector<int> h_block2(D, 1);
...
std::map<std::string, std::vector<int>> buildLanguage(std::map<char, thrust::device_vector<int>> iM)
{
...
double test(std::map<char, thrust::device_vector<int>> iM, std::map<std::string, std::vector<int>> langAM)
{
	double total = 0.0;
	double correct = 0.0;
	double accuracy = 0;
...
int main()
{
	...
	std::vector<int> rand;
	std::map<char, thrust::device_vector<int>> iM;
	std::map<std::string, std::vector<int>> langAM;
	double correct;
